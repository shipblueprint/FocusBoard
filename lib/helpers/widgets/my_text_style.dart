...
class MyTextStyle {
  ...
  // Keep a placeholder to satisfy the original interface without pulling
  // Google Fonts at compile time (which can fail on offline sandboxes).
  static dynamic _fontFamily;
  ...
  static TextStyle getStyle(
    ...
    // `_fontFamily` is null when Google Fonts is disabled. Fall back to a
    // plain TextStyle so text still renders.
    if (_fontFamily == null) {
      return TextStyle(
  ...
  static TextStyle bodyMedium(
    ...
    return getStyle(
        ...
        fontWeight: fontWeight ?? _defaultTextFontWeight[MyTextType.bodyMedium],
        decoration: decoration,
        textStyle: textStyle,
        wordSpacing: wordSpacing,
        xMuted: xMuted);
  ...
  static resetFontStyles() {
    ...
    // Disabled — Google Fonts pulls network assets at compile time and
    // causes `Constant evaluation error` on offline builds. The default
